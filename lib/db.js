...
/** Find tenant by Fonnte device/token */
export function findTenantByDevice(device) {
  return loadTenants().find(t => t.whatsappNumber === device && t.status === 'active') || null;
}

/** Find tenant by Fonnte token */
export function findTenantByToken(token) {
  return loadTenants().find(t => t.fonnteToken === token && t.status === 'active') || null;
}
